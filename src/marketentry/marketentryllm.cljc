(ns marketentry.marketentryllm
  ...
  the MUS public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real PPO e-Procurement System
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-cbris-registration?=" (:has-cbris-registration? e)
                        " eprocurement-registration-verified?=" (:eprocurement-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-cbris-registration? e))
                              (:has-cbris-registration? e))
                          (or (not (:requires-eprocurement-registration? e))
                              (:eprocurement-registration-verified? e)))
                   0.9 0.3)}))
