...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MUS, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up cbris-registration-missing) ==")
    ...
    (println "== filing/submit eng-4 (cbris-registration-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up eprocurement-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (eprocurement-registration-unverified -> HARD hold) ==")
