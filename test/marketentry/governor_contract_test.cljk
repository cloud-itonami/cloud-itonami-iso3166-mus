  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Grand Baie Digital Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Grand Baie Digital Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest cbris-registration-missing-is-held-and-unoverridable
  (testing "missing CBRIS company registration -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:cbris-registration-missing} (-> (store/ledger db) last :basis)))
...
(deftest eprocurement-registration-unverified-is-held-and-unoverridable
  (testing "unverified PPO e-Procurement System registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:eprocurement-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
